...
//   const {products} = productReducer;
  ...
  function deleteHandler(product_id)
  {
    ...
    <div className='cardContainer flex flex-wrap gap-10 my-10'>
      {
        products.map((item) => {
            ...
            <div key={item.id} className='relative card w-64 rounded-md bg-zinc-700 text-white px-2'>
                ...
                <button onClick={()=> deleteHandler(item.id)} className='bottom-2 left-2 mt-2 mb-2 px-8 py-2 bg-red-700 rounded-full font-semibold text-md cursor-pointer'>Remove</button>
            </div>
            )
        })
      }
